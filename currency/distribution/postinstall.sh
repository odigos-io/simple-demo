...
# 2) On Debian/Ubuntu only: install pool into distro php-fpm pool.d (RHEL uses our config via service directly)
...
# 2.5) On RHEL with SELinux: allow nginx to bind to 8085, and allow PHP-FPM to connect to geolocation (8086)
